...
router.post("/sales-cone/cell-property", function(req , res) {
    ...
    let query = `
        ...
        MEMBER [Оборачиваемость] AS SUM(TAIL(EXISTING([Даты].[Дата].[Дата]), 1),   [Measures].[Коэф оборачиваемости])
        ...
        SELECT {[Цена продажи], [Сумма продажи], [Маржа %], [Остаток], [Плановый УВМ], [Оборачиваемость], [Сумма списания], [Доля списания к ТО]} ON 0
        ,[Товары].[Товары] ON 1
