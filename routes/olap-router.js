...
const dailyRevenueFieldPrefix =
    ...
    MEMBER [Остаток артикулов] as [Остаток количество] 
    SELECT {
    ...
    ,[Measures].[Средняя покупка],[Measures].[Кол артикулов],[Measures].[Кол артикулов на 1 клиента], [Measures].[Остаток артикулов],[Measures].[Остаток сумма без НДС],[Measures].[Коэф оборачиваемости]
