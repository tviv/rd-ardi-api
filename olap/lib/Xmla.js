...
function _decodeXmlaTagName(tagName) {
    return tagName.replace(/_x([0-9A-F]{4})_/g, function(match, hex, offset){ //tvv change pattern (was _x(\d\d\d\d)_, A-F was not matched)
        ...
        func = function(node, tagName){
            var list = [],
                criteria = tagName === "*" ? null : function(node){
                    return (node.nodeType === 1 && ((node.namespaceURI === "" || node.prefix === "" ? "" : node.prefix + ":") + node.nodeName) === tagName); //tvv taking account when prefix is empty and then requiring to remove ":"
                }
            ;
            _getElements(node, list, criteria);
            return list;
...
function _xjs(xml) {
  ...
  var re = /<(((([\wа-яА-Я\-\.]+):)?([\wа-яА-Я\-\.]+))([^>]+)?|\/((([\wа-яА-Я\-\.]+):)?([\wа-яА-Я\-\.]+))|\?([\wа-яА-Я]+)([^\?]+)?\?|(!--([^\-]|-[^\-])*--))>|([^<>]+)/ig, //tvv added русский for other languages
  ...
  function unescapeEntities(text) {
    return text.replace(/&((\w+)|#(x?)([0-9a-fA-F]+));/g, function(match, g1, g2, g3, g4, idx, str){ //твв русский
      if (g2) {
        var v = ({
          lt: "<",
          gt: ">",
  ...
  while (match = re.exec(xml)) {
      ...
      if (name = match[5]) {
          ...
          if (atts = match[6]) {
              ...
              var attMatch, att;
              //           123          4               5 6         7
              var attRe = /((([\wа-яА-Я\-]+):)?([\wа-яА-Я\-]+))\s*=\s*('([^']*)'|"([^"]*)")/g;
 ...
 */
    fetchAsObject: function() {
      ...
      if (this.hasSlicerAxis && (axis = this.getSlicerAxis())) { //tvv added second conditions, sometimes axis is null
        filterAxis =  {
          positions: axis.fetchAllAsObject(),
          hierarchies:  getHierarchyArray(axis)
        }
    ...
    _initHierarchies: function(_axisInfoNode){
        ...
        for (i = 0; i < numHierarchies; i++){
            ...
            for (j = 0; j < numPropertyNodes; j++) {
                ...
                // if (this.options.shortHierahPropertyName) { //tvv
                //     if (match = /\[([^.\]]*)\]$/g.exec(memberProperties[nodeName].name) && match.length > 1) {
                //         memberProperties[nodeName].name = match[1];
                //     }
                // }
            }
